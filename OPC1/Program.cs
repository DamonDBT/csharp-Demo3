...
namespace OPC1
{
    class Program
    {
        ...
        {
            ClientHelper.GetOPCServerName("DBT");
            OPCServer opcserver = ClientHelper.ConnectToServer("ArchestrA.DASSIDirect.3");
            var info = ClientHelper.GetServerInfo(opcserver);
            Console.WriteLine(info.StartTime);

            var browser = ClientHelper.RecurBrowse(opcserver);
